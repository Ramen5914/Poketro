...
function Game:start_run(args)
	...
	hud_ante.children[2].children[1].config.object = DynaText({
		string = { { ref_table = PKTR.game_state, ref_value = "lives" } },
...
function PKTR.UI.update_blind_HUD(aniamte)
	if PKTR.is_in_lobby() and PKTR.is_pvp_boss() then
		if aniamte then
		...
		G.E_MANAGER:add_event(Event({
			...
			blockable = false,
			func = function()
				local nemesis = PKTR.get_nemesis()
				...
				G.HUD_blind:get_UIE_by_ID("HUD_blind_count").config.func = "poketro_blind_chip_UI_scale"
			...
			if PKTR.is_pvp_boss() then
				G.E_MANAGER:add_event(Event({
					...
					func = function()
						...
						PKTR.UI.update_blind_HUD(true)
						G.E_MANAGER:add_event(Event({
							trigger = "after",
							delay = 0.45,
							blockable = false,
							func = function()
								if PKTR.lobby_state.config.gamemode == "nemesis" then
									G.HUD_blind:get_UIE_by_ID("HUD_blind_name").config.object.config.string = {
										{
											ref_table = PKTR.get_nemesis(),
											ref_value = "username",
										},
									}
									G.HUD_blind:get_UIE_by_ID("HUD_blind_name").config.object:update_text()
...
function Game:update_shop(dt)
	if not G.STATE_COMPLETE then
		PKTR.game_state.ready_blind = false
		PKTR.game_state.ready_blind_text = localize("ready")
	end
...
function Game:update_hand_played(dt)
	...
	if not PKTR.is_in_lobby() or not PKTR.is_pvp_boss() then
	...
	if not G.STATE_COMPLETE then
		...
		G.E_MANAGER:add_event(Event({
			trigger = "immediate",
			func = function()
				PKTR.send.play_hand(G.GAME.chips, G.GAME.current_round.hands_left)
				-- Set blind chips to enemy score
				G.GAME.blind.chips = PKTR.get_nemesis().score
...
function Game:update_new_round(dt)
	if PKTR.is_in_lobby() and not G.STATE_COMPLETE then
		-- Prevent player from losing
		if to_big(G.GAME.chips) < to_big(G.GAME.blind.chips) and not PKTR.is_pvp_boss() then
			G.GAME.blind.chips = -1
			PKTR.send.fail_round()
		end
...
function Game:update_selecting_hand(dt)
	if
		...
		and #G.deck.cards < 1
		and #G.play.cards < 1
		and PKTR.is_in_lobby()
	then
		G.GAME.current_round.hands_left = 0
		if not PKTR.is_pvp_boss() then
			G.STATE_COMPLETE = false
			G.STATE = G.STATES.NEW_ROUND
		else
			PKTR.send.play_hand(G.GAME.chips, 0)
			G.STATE_COMPLETE = false
...
G.FUNCS.can_open = function(e)
	if PKTR.game_state.ready_blind then
		e.config.colour = G.C.UI.BACKGROUND_INACTIVE
		e.config.button = nil
		return
	end
...
G.FUNCS.poketro_blind_chip_UI_scale = function(e)
	local nemesis = PKTR.get_nemesis()
...
function Game:update_shop(dt)
	...
	if PKTR.is_in_lobby() and not G.STATE_COMPLETE and not updated_location and not G.GAME.USING_RUN then
		PKTR.send.set_location("loc_shop")
		--show_enemy_location()
	end
...
function Game:update_blind_select(dt)
	local updated_location = false
	if PKTR.is_in_lobby() and not G.STATE_COMPLETE and not updated_location then
		PKTR.send.set_location("loc_selecting")
		--show_enemy_location()
...
function G.FUNCS.select_blind(e)
	PKTR.game_state.prevent_eval = false
	select_blind_ref(e)
	if PKTR.is_in_lobby() then
		local blind_key = (e.config.ref_table.key or e.config.ref_table.name)
		PKTR.game_state.ante_key = tostring(math.random())
		PKTR.send.play_hand(0, G.GAME.round_resets.hands)
		PKTR.send.set_location("loc_playing-" .. blind_key)
		--hide_enemy_location()
		if PKTR.is_host() and PKTR.value_is_pvp_boss(blind_key) then
			G.E_MANAGER:add_event(Event({
				trigger = "immediate",
				blockable = false,
				blocking = false,
				no_delete = true,
				func = function()
					local losers = PKTR.get_horde_losers()
					if losers ~= nil then
						for _, v in ipairs(losers) do
							PKTR.send.lose_life(v.code)
						end
						PKTR.send.end_pvp()
						return true
					end
...
G.FUNCS.skip_blind = function(e)
	skip_blind_ref(e)
	if PKTR.is_in_lobby() then
		PKTR.send.set_skips(G.GAME.skips)
	end
end
