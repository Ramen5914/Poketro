...
function create_UIBox_blind_choice(type, run_info)
	if PKTR.is_in_lobby() then
		...
		if PKTR.value_is_pvp_boss(G.GAME.round_resets.blind_choices[type]) then
			local dt1 = DynaText({
							...
							nodes = {
								not run_info and {
									...
									config = {
										...
										hover = true,
										one_press = true,
										func = PKTR.value_is_pvp_boss(G.GAME.round_resets.blind_choices[type])
		...
		then
			...
			if e.config.id == G.GAME.blind_on_deck then
				...
				if _top_button then
					...
					if _top_button.config.button ~= "pktr_toggle_ready" then
...
G.FUNCS.pvp_ready_button = function(e)
	if e.children[1].config.ref_table[e.children[1].config.ref_value] == localize("Select", "blind_states") then
		e.config.button = "pktr_toggle_ready"
		e.config.one_press = false
		e.children[1].config.ref_table = PKTR.game_state
		e.children[1].config.ref_value = "ready_blind_text"
	end
	if e.config.button == "pktr_toggle_ready" then
		e.config.colour = (PKTR.game_state.ready_blind and G.C.GREEN) or G.C.RED
	end
end

function G.FUNCS.pktr_toggle_ready(e)
	PKTR.game_state.ready_blind = not PKTR.game_state.ready_blind
	PKTR.game_state.ready_blind_text = PKTR.game_state.ready_blind and localize("ready") or localize("unready")

	if PKTR.game_state.ready_blind then
		--G.MULTIPLAYER.set_location("loc_ready")
		PKTR.send.ready_blind(e)
	else
		--G.MULTIPLAYER.set_location("loc_selecting")
		PKTR.send.unready_blind()
	end
end
